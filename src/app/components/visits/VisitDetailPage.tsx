...
export function VisitDetailPage() {
  ...
  const [isRealVisit, setIsRealVisit] = useState(false);
  ...
  const handleDeleteVisit = async () => {
    if (!isRealVisit || isDeletingVisit) return;
      ...
      <div className="px-4 mt-4 flex flex-col gap-3">
        ...
        {activeTab === "overview" && (
          <>
            ...
            {isRealVisit && (
              <div className="rounded-2xl p-3.5" style={{ background: "#FFF1F2", border: "1px solid #FECDD3" }}>
                <button
                  onClick={() => { void handleDeleteVisit(); }}
                  disabled={isDeletingVisit}
                  className="w-full py-3 rounded-xl transition-all"
                  style={{
                    background: isDeletingVisit ? "#FCA5A5" : "#DC2626",
                    color: "white",
                    fontSize: "0.82rem",
                    fontWeight: 700,
                    opacity: isDeletingVisit ? 0.8 : 1,
                    cursor: isDeletingVisit ? "not-allowed" : "pointer",
                  }}
                >
                  {isDeletingVisit ? "Deleting Visit…" : "Delete Visit"}
                </button>
                {deleteError && (
                  <p style={{ color: "#B91C1C", fontSize: "0.74rem", marginTop: 8, lineHeight: 1.4 }}>
                    {deleteError}
                  </p>
                )}
              </div>
